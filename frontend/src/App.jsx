...
function App() {
  ...
  const [searchQuery, setSearchQuery] = useState(''); // <- THÊM MỚI
  
  // Scroll reveal hooks cho từng section
  const [recipeSuggestionRef, isSuggestionVisible] = useScrollReveal({ threshold: 0.2 });
  const [recipeSearchRef, isSearchVisible] = useScrollReveal({ threshold: 0.2 }); 
